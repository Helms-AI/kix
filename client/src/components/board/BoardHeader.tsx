...
import type { WorkItemType, BoardColumn } from '../../types/project';
import { WORK_ITEM_TYPES, BOARD_COLUMNS } from '../../types/project';
...
interface BoardHeaderProps {
  ...
  filterItemType: WorkItemType | null;
  onFilterItemTypeChange: (type: WorkItemType | null) => void;
  onCreateItem: () => void;
  onRefresh: () => void;
  isLoading?: boolean;
}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            Filter
            {filterItemType && (
              <span className="px-1.5 py-0.5 bg-blue-500/30 rounded text-xs">1</span>
            )}
          </button>
      ...
      {showFilters && (
        <div className="flex items-center gap-3 pt-2 border-t border-slate-800/30">
          ...
          <div className="flex items-center gap-1.5">
            {/* All Types */}
            <button
              onClick={() => onFilterItemTypeChange(null)}
            ...
            {WORK_ITEM_TYPES.map(({ type, label }) => {
              const typeClass = getWorkItemTypeColor(type);
              const icon = getWorkItemTypeIcon(type);
              const isActive = filterItemType === type;
              return (
                <button
                  key={type}
                  onClick={() => onFilterItemTypeChange(isActive ? null : type)}
